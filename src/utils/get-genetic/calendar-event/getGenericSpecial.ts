...
async function getGenericSpecial(
    ...
    console.log("==============================")
    console.log("==============================")
    console.log("mira que es startDate", startDate)
    console.log("mira que es endDate", endDate)
...
function processFilters(filters: Record<string, { value: any; relation?: string, relation2?: string, relationType?: string, isEnum?: boolean, }>) {
    let where: any = {};
    let orConditions: any[] = [];

    Object.entries(filters).forEach(([field, filter]) => {
        const { value, relation, relation2, relationType = 'is', isEnum } = filter; // Cambiar a 'is' por defecto
        const isEnumField = field.endsWith('Type') || isEnum;
        ...
        if (!relation) {
            ...
            if (Array.isArray(value)) {
                value.forEach(val => {
                    const condition = isEnumField ? { equals: val } : (typeof val === 'string' ? { contains: val, mode: 'insensitive' } : { equals: val });
                    if (relation2) {
                        orConditions.push({ [relation2]: { [relation]: { [relationType]: { [actualField]: condition } } } });
                    } else {
                        orConditions.push({ [relation]: { [relationType]: { [actualField]: condition } } });
                    }
                });
            } else {
                const condition = isEnumField ? { equals: value } : (typeof value === 'string' ? { contains: value, mode: 'insensitive' } : { equals: value });
                if (relation2) {
                    where[relation2] = { [relation]: { [relationType]: { [actualField]: condition } } };
                } else {
                    where[relation] = { [relationType]: { [actualField]: condition } };
                }
            }
        }
    });
